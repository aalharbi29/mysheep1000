...
import { Baby, Calendar, Palette, TreePine, Plus, Edit } from 'lucide-react';

const AnimalDetailPage = () => {
  const { id } = useParams<{ id: string }>();
  const { getAnimalById, updateAnimal, addBirthRecord } = useLivestock();
  ...
  const [offspringList, setOffspringList] = useState<Partial<Offspring>[]>([
    { gender: 'female', color: 'أبيض', fate: 'flock' },
  ]);
  ...
  const handleAddBirth = () => {
    const record: BirthRecord = {
      ...
      date: birthDate,
      offspring: offspringList.map((o, i) => ({
        id: `${Date.now()}-${i}`,
        number: 0,
    ...
    setBirthOpen(false);
    setBirthDate('');
    setOffspringList([{ gender: 'female', color: 'أبيض', fate: 'flock' }]);
  };

  const addOffspringField = () => {
    setOffspringList(prev => [...prev, { gender: 'female', color: 'أبيض', fate: 'flock' }]);
  };

  const updateOffspringField = (index: number, field: string, value: string) => {
    ...
    <div className="min-h-screen bg-background p-4 sm:p-6">
      <div className="max-w-2xl mx-auto">
        <PageHeader
          ...
          subtitle={CATEGORY_LABELS[animal.breed] || animal.breed}
          backTo={backPath}
        />
                ...
                <div key={record.id} className="rounded-xl bg-card p-4 card-shadow">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {record.offspring.map((off) => (
                      ...
                      >
                        <span className="font-bold">{GENDER_LABELS[off.gender]}</span>
